...
## Phase 6 — Polish & performance ✅ done (BL-15..BL-20)

- [x] Pagination added to `home` and `Biography_views` (`Paginator`, 12/page). Caught and fixed a real bug in `_pagination.html` before it shipped: naively reading `previous_page_number`/`next_page_number` raises an uncaught `EmptyPage` on the first/last page — `EmptyPage` has no `silent_variable_failure`, so Django's template engine doesn't swallow it. Guarded with `{% if has_previous/has_next %}`. Used `Paginator.get_page()` (clamps `page=999`/`page=0`/`page=abc` instead of raising).
- [x] `select_related('coffee_item')` was already present from BL-05, but writing the query-count regression test BL-16 asked for found a real N+1 anyway: `cart_detail`'s `total_price` came from `Cart.total_price`, a model property that ran its own separate, unoptimized `cart.items.all()` query. Fixed by computing the total from the already-fetched, already-optimized list instead. Confirmed the new test has teeth by reverting the fix locally and watching it fail.
- [x] Bootstrap CSS/JS and Font Awesome CSS pinned with real (not fabricated) SHA-384 SRI hashes — downloaded the exact CDN files and computed the digests directly, re-verified with an independent second fetch.
- [x] The three visual styles consolidated into `static/css/style.css`. Not a naive copy-paste: `login.html`/`register.html` both defined `.form-submit` differently, and `biography.html`'s bare `table`/`th`/`td` would have leaked into `cart_detail.html`'s Bootstrap table once global — scoped everything under a new `body_class` block per page (`page-coffee`/`page-biography`/`page-login`/`page-register`).
- [x] Duplicate URL mount resolved: dropped `/coffee/`, kept root `''`. Verified `reverse()` already resolved every `{% url %}` call to the root paths before touching anything (Django's resolver has the later-registered pattern win for a shared name) and grepped for hardcoded `/coffee/` references (none) — zero-regression change, confirmed `/coffee/...` now correctly 404s.
- [x] `django-debug-toolbar` added, gated behind `DEBUG` (and excluded from `manage.py test` runs). Verified it actually catches an N+1, not just that it's installed: watched its own query-count badge stay flat at 4 for a 5-item cart (post BL-16 fix), then jump to 9 after temporarily reverting that fix, before restoring it.
